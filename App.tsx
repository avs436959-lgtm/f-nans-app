...
export default function App() {
  // 💾 İLK AÇILIŞTA TARAYICI HAFIZASINDAN VERİLERİ ÇEKİYORUZ
  const [transactions, setTransactions] = useState<Transaction[]>(() => {
    const saved = localStorage.getItem('finans_islemler');
    if (saved) {
      return JSON.parse(saved);
    }
    // Eğer hafıza boşsa ilk başta görünecek örnek veriler
    return [
      { id: '1', title: 'Maaş Başlangıç', amount: 45000, type: 'income', category: 'Maaş', date: '2026-07-01' },
  ...
  // 💾 VERİLER HER DEĞİŞTİĞİNDE TARAYICI HAFIZASINA OTOMATİK KAYDEDİYORUZ
  useEffect(() => {
    localStorage.setItem('finans_islemler', JSON.stringify(transactions));
  }, [transactions]);
              ...
              <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl h-fit">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Örn: Market" className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-white focus:outline-none text-sm" />
                  </div>
                  <div>
